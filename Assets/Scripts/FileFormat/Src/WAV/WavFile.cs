...
namespace FileFormat
{
    public class WavFile
    {
        ...
        {
            new WavFmtChunk(format, channels, sampleRate)
        };
            if (format != WavFormat.PCM)
            {
                chunks.Add(new WavFactChunk(samples.Length));
            }
            chunks.Add(new WavDataChunk(format, channels, samples));

            var riffChunk = new WavRiffChunk(chunks);
    ...
    public class WavFmtChunk : WavChunk
    {
        ...
        public WavFmtChunk(WavFormat format, int channels, int sampleRate) : base("fmt ")
        {
            this.format = format;
            this.channels = channels;
            this.sampleRate = sampleRate;
    ...
    public class WavFactChunk : WavChunk
    {
        private readonly int sampleCount;

        protected override int InnerChunkSize => 4;

        public WavFactChunk(int sampleCount) : base("fact")
        {
    ...
    public class WavDataChunk : WavChunk
    {
        private readonly WavFormat format;
        private readonly int channels;
        ...
        public WavDataChunk(WavFormat format, int channels, float[] samples) : base("data")
        {
            this.format = format;
            this.channels = channels;
            this.samples = samples;
